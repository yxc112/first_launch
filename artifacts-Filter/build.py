...
from dict import eff_simple, main_simple, set_simple
...
def load_build(reload=False):
    build_pkl = Path(proDir + '/config/expend_build.pkl')
    if (not build_pkl.exists()) or reload:
        raw_build_data = pd.read_csv(proDir + '/config/build.csv',encoding='utf8')
        raw_build_data['角色'] = raw_build_data['角色'].apply(lambda x:str(x))
        
        print('正在展开build......')
        d = []
        for i,row in tqdm(raw_build_data.iterrows()):
            if row['启用'] == 'yes':
                d.append(expand(row))
        build_df = pd.DataFrame(d)
        build_df.to_pickle(proDir + '/config/expend_build.pkl')
        print('展开完成.')
    build_df = pd.read_pickle(proDir + '/config/expend_build.pkl')
    return build_df

if __name__ == '__main__':
    load_build(reload=True)
