attribute_dict = {
    "attackPercentage": {
        "chs": "攻击力%",
        "average": 0.04955
    },
    "lifePercentage": {
        "chs": "生命值%",
        "average": 0.04955
    },
    "defendPercentage": {
        "chs": "防御力%",
        "average": 0.06195
    },
    "elementalMastery": {
        "chs": "元素精通",
        "average": 19.815
    },
    "recharge": {
        "chs": "元素充能效率",
        "average": 0.05505
    },
    "thunderBonus": {
        "chs": "雷元素伤害加成",
    },
    "fireBonus": {
        "chs": "火元素伤害加成",
    },
    "waterBonus": {
        "chs": "水元素伤害加成",
    },
    "iceBonus": {
        "chs": "冰元素伤害加成",
    },
    "windBonus": {
        "chs": "风元素伤害加成",
    },
    "rockBonus": {
        "chs": "岩元素伤害加成",
    },
    "dendroBonus": {
        "chs": "草元素伤害加成",
    },
    "physicalBonus": {
        "chs": "物理元素伤害加成",
    },
    "critical": {
        "chs": "暴击率",
        "average": 0.03305
    },
    "criticalDamage": {
        "chs": "暴击伤害",
        "average": 0.06605
    },
    "cureEffect": {
        "chs": "治疗加成",
    },
    "attackStatic": {
        "chs": "攻击力",
        "average": 16.535
    },
    "lifeStatic": {
        "chs": "生命值",
        "average": 253.94
    },
    "defendStatic": {
        "chs": "防御力",
        "average": 19.675
    }
}

set_dict = {
    "adventurer": {
        "eng": "adventurer",
        "chs": "冒险家"},
    "archaicPetra": {
        "eng": "archaicPetra",
        "chs": "悠古的磐岩"},
    "berserker": {
        "eng": "berserker",
        "chs": "战狂"},
    "blizzardStrayer": {
        "eng": "blizzardStrayer",
        "chs": "冰风迷途的勇士"},
    "bloodstainedChivalry": {
        "eng": "bloodstainedChivalry",
        "chs": "染血的骑士道"},
    "braveHeart": {
        "eng": "braveHeart",
        "chs": "勇士之心"},
    "crimsonWitchOfFlames": {
        "eng": "crimsonWitchOfFlames",
        "chs": "炽烈的炎之魔女"},
    "defenderWill": {
        "eng": "defenderWill",
        "chs": "守护之心"},
    "echoesOfAnOffering": {
        "eng": "echoesOfAnOffering",
        "chs": "来歆余响"},
    "emblemOfSeveredFate": {
        "eng": "emblemOfSeveredFate",
        "chs": "绝缘之旗印"},
    "gambler": {
        "eng": "gambler",
        "chs": "赌徒"},
    "gladiatorsFinale": {
        "eng": "gladiatorsFinale",
        "chs": "角斗士的终幕礼"},
    "heartOfDepth": {
        "eng": "heartOfDepth",
        "chs": "沉沦之心"},
    "huskOfOpulentDreams": {
        "eng": "huskOfOpulentDreams",
        "chs": "华馆梦醒形骸记"},
    "instructor": {
        "eng": "instructor",
        "chs": "教官"},
    "lavawalker": {
        "eng": "lavawalker",
        "chs": "渡过烈火的贤人"},
    "luckyDog": {
        "eng": "luckyDog",
        "chs": "幸运儿"},
    "maidenBeloved": {
        "eng": "maidenBeloved",
        "chs": "被怜爱的少女"},
    "martialArtist": {
        "eng": "martialArtist",
        "chs": "武人"},
    "noblesseOblige": {
        "eng": "noblesseOblige",
        "chs": "昔日宗室之仪"},
    "oceanHuedClam": {
        "eng": "oceanHuedClam",
        "chs": "海染砗磲"},
    "paleFlame": {
        "eng": "paleFlame",
        "chs": "苍白之火"},
    "prayersForWisdom": {
        "eng": "prayersForWisdom",
        "chs": "祭雷之人"},
    "prayersToSpringtime": {
        "eng": "prayersToSpringtime",
        "chs": "祭冰之人"},
    "prayersForIllumination": {
        "eng": "prayersForIllumination",
        "chs": "祭火之人"},
    "prayersForDestiny": {
        "eng": "prayersForDestiny",
        "chs": "祭水之人"},
    "resolutionOfSojourner": {
        "eng": "resolutionOfSojourner",
        "chs": "行者之心"},
    "retracingBolide": {
        "eng": "retracingBolide",
        "chs": "逆飞的流星"},
    "scholar": {
        "eng": "scholar",
        "chs": "学士"},
    "shimenawaReminiscence": {
        "eng": "shimenawaReminiscence",
        "chs": "追忆之注连"},
    "shimenawasReminiscence": {
        "eng": "shimenawasReminiscence",
        "chs": "追忆之注连"},
    "tenacityOfTheMillelith": {
        "eng": "tenacityOfTheMillelith",
        "chs": "千岩牢固"},
    "exile": {
        "eng": "exile",
        "chs": "流放者"},
    "thunderingFury": {
        "eng": "thunderingFury",
        "chs": "如雷的盛怒"},
    "thunderSmoother": {
        "eng": "thunderSmoother",
        "chs": "平息鸣雷的尊者"},
    "tinyMiracle": {
        "eng": "tinyMiracle",
        "chs": "奇迹"},
    "travelingDoctor": {
        "eng": "travelingDoctor",
        "chs": "游医"},
    "VermillionHereafter": {
        "eng": "VermillionHereafter",
        "chs": "辰砂往生录"},
    "vermillionHereafter": {
        "eng": "vermillionHereafter",
        "chs": "辰砂往生录"},
    "viridescentVenerer": {
        "eng": "viridescentVenerer",
        "chs": "翠绿之影"},
    "wandererTroupe": {
        "eng": "wandererTroupe",
        "chs": "流浪大地的乐团"},
    "wanderersTroupe": {
        "eng": "wanderersTroupe",
        "chs": "流浪大地的乐团"},
    "DeepwoodMemories": {
        "eng": "DeepwoodMemories",
        "chs": "深林的记忆"},
    "deepwoodMemories": {
        "eng": "deepwoodMemories",
        "chs": "深林的记忆"},
    "GildedDreams": {
        "eng": "GildedDreams",
        "chs": "饰金之梦"},
    "gildedDreams": {
        "eng": "gildedDreams",
        "chs": "饰金之梦"}
}

set_simple = {'如雷': ['thunderingFury'],
              '魔女': ['crimsonWitchOfFlames'],
              '水套': ['heartOfDepth'],
              '冰风': ['blizzardStrayer'],
              '风套': ['viridescentVenerer'],
              '磐岩': ['archaicPetra'],
              '草套': ['deepwoodMemories'],
              '苍白': ['paleFlame'],
              '骑士': ['bloodstainedChivalry'],
              '绝缘': ['emblemOfSeveredFate'],
              '乐团': ['wanderersTroupe'],
              '饰金': ['gildedDreams'],
              '宗室': ['noblesseOblige'],
              '海染': ['oceanHuedClam'],
              '少女': ['maidenBeloved'],
              '追忆': ['shimenawasReminiscence'],
              '余响': ['echoesOfAnOffering'],
              '角斗士': ['gladiatorsFinale'],
              '辰砂': ['vermillionHereafter'],
              '千岩': ['tenacityOfTheMillelith'],
              '华馆': ['huskOfOpulentDreams'],
              '逆飞': ['retracingBolide'],
              '贤人': ['lavawalker'],
              '尊者': ['thunderSmoother'],
              '勇士之心': ['braveHeart'],
              '行者之心': ['resolutionOfSojourner'],
              '守护之心': ['defenderWill'],
              '教官': ['instructor'],
              '流放者': ['exile'],
              '战狂': ['berserker'],
              '武人': ['martialArtist'],
              '赌徒': ['gambler'],
              '学士': ['scholar'],
              '奇迹': ['tinyMiracle'],
              '祭雷': ['prayersForWisdom'],
              '祭火': ['prayersForIllumination'],
              '祭水': ['prayersForDestiny'],
              '祭冰': ['prayersToSpringtime'],
              '冒险家': ['adventurer'],
              '幸运儿': ['luckyDog'],
              '游医': ['travelingDoctor'],
              '攻击2': ['shimenawasReminiscence', 'echoesOfAnOffering', 'gladiatorsFinale', 'vermillionHereafter',
                        'braveHeart', 'resolutionOfSojourner'],
              '精通2': ['wanderersTroupe', 'gildedDreams', 'instructor'],
              '防御2': ['huskOfOpulentDreams', 'defenderWill']
              }

main_simple = {
    '生': ['lifePercentage'],
    '攻': ['attackPercentage'],
    '防': ['defendPercentage'],
    '精': ['elementalMastery'],
    '充': ['recharge'],
    '雷': ['thunderBonus'],
    '火': ['fireBonus'],
    '水': ['waterBonus'],
    '冰': ['iceBonus'],
    '风': ['windBonus'],
    '岩': ['rockBonus'],
    '草': ['dendroBonus'],
    '物': ['physicalBonus'],
    '暴率': ['critical'],
    '暴伤': ['criticalDamage'],
    '双暴': ['critical', 'criticalDamage'],
    '治': ['cureEffect']
}

eff_simple = {
    '攻击%': 'attackPercentage',
    '生命%': 'lifePercentage',
    '防御%': 'defendPercentage',
    '精通': 'elementalMastery',
    '充能': 'recharge',
    '暴率': 'critical',
    '暴伤': 'criticalDamage'
}

position_dict = {
    'flower': '生之花',
    'feather': '死之羽',
    'sand': '时之沙',
    'cup': '空之杯',
    'head': '理之冠',
}